...
def load_index(index_path: Path, nprobe: int) -> indexing.Index:
    ...
    try:
        ...
        ivf = None
    if ivf is not None:
        # If lists are not OnDisk, try to reattach using the same prefix used at indexing time
        try:
        ...
        if not on_disk_already:
            # The builder used <ONDISK_DIR>/ivf.lists as the filename prefix
            list_prefix = str((ONDISK_DIR / "ivf.lists").resolve())
            # Only attempt if those files exist (prefix presence heuristic)
            ...
            if prefix_exists:
                ...
                # hand ownership to FAISS C++
                od.thisown = False
                ivf.replace_invlists(od, True)
            else:
                print("Warning: On-disk list prefix not found; proceeding with whatever is embedded in index.")
